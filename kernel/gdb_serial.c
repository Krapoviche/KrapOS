...
#define PUTERR(code)	do { errchar = (code); goto errpacket; } while (0)
	...
	do {
		...
		case 'H':	/* set thread for following operations */
		{
			...
			if (th != (unsigned)-1 && th != 0u && th != p->thread_id) {
		...
		case '?':
			if (cb && cb->thread_switch &&
			    p->thread_id != 0 && p->thread_id != (unsigned)-1) {
				/*
				...
				inbuf[x++] = ';';
				inbuf[x] = 0;
				putpacket({ (unsigned char *)"T", 0, 0 }, 
                                          { &p->signo, 1, 1 },
                                          { (unsigned char *)inbuf, 0, 0 });
			}
			else
				putpacket({(unsigned char *)"S", 0, 0}, {&p->signo,1, 1 });
			break;
		...
		case '!':
			...
			/* FALLTHROUGH */
		ok:
		putpacket({(unsigned char *)"OK", 0, 0});
		break;

		errpacket:	/* error replies jump to here */
		putpacket({(unsigned char *)"E", 0, 0}, {&errchar, 1, 1});
		break;
...
void gdb_serial_exit(int exit_code)
{
	...
	putpacket({(unsigned char *)"W", 0, 0}, {&exchar, 1, 1});
	connected = 0;
...
void gdb_serial_killed(int signo)
{
	...
	/* Indicate that we have terminated.  */
	putpacket({(unsigned char *)"X", 0, 0}, {&exchar,1,1});
	connected = 0;
...
void gdb_serial_putchar(int ch)
{
	...
	putpacket ({(unsigned char *)"O", 0, 0}, {&uch,1,1});
	base_critical_leave();
}
...
void gdb_serial_puts(const char *s)
{
	...
	putpacket({(unsigned char *)"O", 0, 0}, {(unsigned char *)s,0,1}, {(unsigned char *)"\n",1,1});
	base_critical_leave();
}
